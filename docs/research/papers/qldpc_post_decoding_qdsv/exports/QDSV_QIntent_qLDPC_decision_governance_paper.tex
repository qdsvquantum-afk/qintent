...
\title{QDSV/QIntent as a Guarded Semantic Decision Layer for LDPC/qLDPC-Style Post-Decoding Correction Governance}
\author{QDSV/QIntent Research Draft}
\date{\today}
...
\item Does the candidate match the syndrome?
\item How confident is the decoder?
\item Do multiple decoders agree?
\item Does the correction touch structurally sensitive qubits?
\item Does it increase propagation risk?
\item Is it likely to preserve the logical subspace?
\item Can the decision be audited and reproduced?
\end{itemize}
...
\item It formulates post-decoding correction selection as a guarded semantic decision-governance problem rather than as a replacement decoder.
\item It evaluates QDSV/QIntent over synthetic ambiguity, BP-soft evidence and external \texttt{ldpc} decoder outputs including BP, BP+OSD and BP+LSD.
\item It introduces guarded override policies that reduce harmful overrides while preserving the ability to recover from baseline decoder failures.
\item It validates logical preservation in two known stabilizer-code settings by computing the residual \texttt{R = C E} and classifying it against \texttt{S} and \texttt{N(S)\textbackslash{}S}.
\end{enumerate}
...
\section{Gap Analysis in LDPC/qLDPC Decoding}
...
\midrule
Syndrome ambiguity & Multiple correction hypotheses can satisfy the same syndrome. & Represents candidates as decision states and evaluates syndrome, decoder and logical-safety evidence together. & Add hardware-derived syndrome-count evidence and compare Aer vs IBM observed syndromes. & \textasciitilde{}60\% & \textasciitilde{}70\% \\
Candidate selection under competing criteria & The highest-confidence or minimum-weight correction may not be the safest correction. & Compares baseline, raw QDSV and guarded QDSV using structured evidence, confidence, risk and safety signals. & Add oracle-best and policy-ablation baselines. & \textasciitilde{}78\% & \textasciitilde{}85\% \\
Risk-aware correction choice & A locally plausible correction can be structurally risky or logical-sensitive. & Adds logical-risk, propagation-safety, distance-safety and logical-preservation proxies; guarded QDSV prevents unsafe overrides. & Replace part of the proxy logic with explicit logical-observable or stabilizer-derived sensitivity features. & \textasciitilde{}72\% & \textasciitilde{}80\% \\
Decoder disagreement & BP, BP+OSD, BP+LSD or alternative methods may produce different corrections. & Treats decoder outputs as an ensemble and selects across them using method reliability, risk and guarded override evidence. & Compare against BP+OSD-preferred, BP+LSD-preferred and oracle-best ensemble baselines. & \textasciitilde{}65\% & \textasciitilde{}78\% \\
Auditability and reproducibility & Decoder decisions often lack a structured, human-readable decision trace. & Produces public block-level evidence, reproducible JSON/CSV traces and IBM job/backend metadata. & Add repeated hardware runs and cross-backend comparison. & \textasciitilde{}86\% & \textasciitilde{}90\% \\
Evidence insufficiency detection & Some cases cannot be resolved with the available signals. & Ambiguity audit now includes low-margin, decoder-disagreement, evidence-insufficient flags, IBM syndrome-count dispersion and guarded rejection reasons. & Add repeated hardware runs and uncertainty calibration. & \textasciitilde{}70\% & \textasciitilde{}78\% \\
Real-time latency & Decoding must eventually run under strict timing constraints. & Offline timing instrumentation now reports decode, candidate generation, QDSV scoring and total policy time. & Compare local timing against Colab/IBM-derived workflow timings and identify real-time-compatible substeps. & \textasciitilde{}25\% & \textasciitilde{}35\% \\
Full logical-operator preservation & A real qLDPC system requires formal logical operator analysis, not proxies. & Adds two known stabilizer-code validations, 5-qubit and Steane CSS, using residual classification \texttt{R = C E}, with \texttt{R in S} as logical success and \texttt{R in N(S) \textbackslash{} S} as logical failure. & Extend from correctable single-error cases to larger stabilizer/CSS examples and production qLDPC-style codes. & \textasciitilde{}65\% & \textasciitilde{}78\% \\
Hardware noise and measurement faults & Real devices include correlated noise, readout errors and time dynamics. & Preliminary IBM \texttt{ibm\_fez} syndrome-count evidence is archived for four small syndrome scenarios. & Repeat on additional backends and add noisy repeated runs. & \textasciitilde{}40\% & \textasciitilde{}55\% \\
Production qLDPC code families & Results should be tested on known production-relevant qLDPC constructions. & Adds named 5-qubit and Steane CSS stabilizer-code benchmarks with explicit stabilizer generators, normalizer and residual classification; qLDPC-scale families remain future work. & Add surface-code and production qLDPC-style benchmarks. & \textasciitilde{}58\% & \textasciitilde{}70\% \\
\bottomrule
\end{longtable}
\end{small}


Overall estimated coverage of the current work: approximately 72-80\% of the broader qLDPC decoding workflow gap after the external \texttt{ldpc} ensemble benchmark, uncertainty instrumentation, timing instrumentation, guarded policy evaluation, preliminary IBM hardware-oriented syndrome validation and two known stabilizer-code logical validations.
...
\section{QDSV/QIntent Post-Decoding Model}
...
\begin{small}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.320\textwidth}>{\raggedright\arraybackslash}p{0.320\textwidth}}
\toprule
...
\subsection{Experiment 1: Controlled Ambiguity Benchmark}
...
\subsection{Experiment 2: Random Sparse Benchmark with Ambiguity Audit}
...
\subsection{Experiment 3: BP-Soft Multi-Seed Benchmark}
...
\subsection{Experiment 4: External \texttt{ldpc} Decoder-Ensemble Recovery}
...
\subsection{Experiment 5: IBM Hardware-Oriented Syndrome Validation}
...
\subsection{Experiment 6: Known Stabilizer-Code Logical Validation}
...
For each physical error \texttt{E} and selected correction \texttt{C}, the experiment computes:
...
The benchmark evaluates a confidence-only baseline, raw QDSV/QIntent, guarded QDSV/QIntent and an \texttt{oracle\_best} reference over the same candidate set. The oracle is used only as an experimental upper-bound reference; it is not part of the proposed method.


\subsection{Experiment 7: Steane CSS Logical Validation}
...
\subsection{Controlled Ambiguity Benchmark}
...
\subsection{Random Sparse Benchmark}
...
\subsection{BP-Soft Multi-Seed Benchmark}
...
\subsection{External \texttt{ldpc} Decoder-Ensemble Recovery}
...
\subsection{IBM Hardware-Oriented Syndrome Validation}
...
\subsection{Known 5-Qubit Stabilizer-Code Logical Validation}
...
\subsection{Steane [[7,1,3]] CSS Logical Validation}
...
\begin{enumerate}[leftmargin=*]
\item Raw QDSV/QIntent can use logical-risk and safety signals to override unsafe confidence-only selections.
\item Guarded QDSV/QIntent can reduce bad overrides and preserve reliable baseline decisions.
\item When evidence is insufficient, ambiguity audits reveal where more decoder information is required.
\item With real external LDPC decoder outputs, QDSV/QIntent can recover from BP failures by selecting across BP, BP+OSD, BP+LSD and compatible alternatives.
\item In known stabilizer-code settings, QDSV/QIntent decisions can be evaluated against formal residual membership conditions rather than only proxy risk metrics.
\item The formal logical-preservation behavior was reproduced across two small code families: a 5-qubit perfect stabilizer code and the Steane CSS code.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
\item The LDPC/qLDPC-style check matrices are sparse synthetic structures; they are useful for controlled decision-governance tests but are not production qLDPC code families.
\item Formal residual classification has so far been demonstrated only on small correctable single-error settings: the 5-qubit perfect stabilizer code and the Steane [[7,1,3]] CSS code.
\item The experiments do not demonstrate scalability to large qLDPC codes, repeated syndrome extraction cycles or full fault-tolerant operation.
\item The experiments are offline and do not establish real-time decoding suitability, even though local QDSV decision timing is instrumented.
\item The external \texttt{ldpc} experiment focuses on BP-failure recovery and does not claim superiority over BP+OSD or BP+LSD as standalone decoders.
\item Raw QDSV can still produce harmful overrides in some settings; guarded QDSV reduces but does not mathematically eliminate all bad override risk.
\item The IBM hardware-oriented run is intentionally small and validates handoff from hardware syndrome counts, not hardware fault tolerance or production qLDPC decoding.
\item The \texttt{oracle\_best} rows are upper-bound references over available candidate sets. They are not deployable baselines and are not part of the proposed method.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
\item Extend formal logical analysis from the 5-qubit and Steane stabilizer benchmarks to surface-code and qLDPC-style code families.
\item Test known LDPC/qLDPC code constructions rather than only synthetic sparse matrices.
\item Tune guarded decision policies against larger decoder ensembles and policy-ablation baselines.
\item Measure latency and identify which parts could run in real-time versus offline audit.
\item Evaluate noisy syndrome/readout conditions using \texttt{SoftInfoBpDecoder} or related workflows.
\item Compare against BP+OSD/BP+LSD as baselines, while keeping the claim focused on post-decoding policy rather than decoder replacement.
\end{enumerate}

\subsection{Hardware-Oriented Validation Status}
