"""Known stabilizer-code logical preservation benchmark for QDSV/QIntent.

This experiment closes the main QEC gap left by the LDPC/qLDPC-style
post-decoding benchmarks: formal logical preservation.

It uses the 5-qubit perfect stabilizer code. For each physical error E and
selected correction C, it computes the residual R = C E and classifies:

- R in stabilizer group S     -> no logical failure
- R in normalizer N(S) \\ S    -> non-trivial logical residual

QDSV is evaluated as a guarded semantic decision layer over candidate
corrections. The private scoring formula is not exposed.
"""
...
find_rows("candidate_index")
  ...
  ], global_risk="logical_risk", profile="five_qubit_stabilizer_logical_validation")
...
class BenchmarkConfig:
    seed_start: int = 510001
    seed_count: int = 8
    samples_per_seed: int = 40
    max_candidate_weight: int = 3
    max_candidates_per_scenario: int = 36
    ambiguity_boost_probability: float = 0.42
...
NORMALIZER = {"".join(chars) for chars in product(PAULIS, repeat=5) if syndrome("".join(chars)) == (0, 0, 0, 0)}
...
def all_candidate_corrections(target_syndrome: tuple[int, ...], max_weight: int) -> list[str]:
    ...
    for w in range(0, max_weight + 1):
        for qubits in combinations(range(5), w):
            if w == 0:
                candidate = "IIIII"
...
class FiveQubitLogicalBenchmark:
    ...
    def sample_error(self) -> str:
        # The 5-qubit code corrects arbitrary single-qubit Pauli errors.
        return pauli_at(self.rng.randrange(5), self.rng.choice(NON_IDENTITY))
    ...
    def build_rows(self, true_error: str, scenario_id: int) -> list[dict[str, Any]]:
        ...
        for candidate in candidates:
            ...
            # Decoder confidence is intentionally noisy: it models a decoder
            # that can occasionally prefer a plausible but logically harmful
            # correction hypothesis.
            confidence = 970 - 135 * max(0, candidate_weight - 1) + self.rng.randint(-45, 45)
            if candidate == true_error:
                confidence += self.rng.randint(0, 45)
            if candidate == boosted:
                confidence += self.rng.randint(170, 260)
            ...
            raw_rows.append(
                {
                    ...
                    "logical_preservation": max(0, 980 - 250 * structural_weight_penalty),
                    "distance_safety": max(0, 950 - 230 * structural_weight_penalty),
                    "decoder_confidence": confidence,
                    "method_reliability": 860,
                    "propagation_safety": max(0, 950 - 110 * candidate_weight),
                    "syndrome_risk": 35 + 10 * candidate_weight,
                    "logical_risk": min(1000, 30 + 150 * structural_weight_penalty + 20 * candidate_weight),
...
def _seed_metrics(seed: int, attempts: int, frame: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "raw_recovery_over_baseline_failure_rate": _rate(
            (baseline_failure & ~frame["qdsv_raw_formal_logical_failure"].astype(bool))[baseline_failure]
        ),
        "guarded_recovery_over_baseline_failure_rate": _rate(
            (baseline_failure & ~frame["qdsv_guarded_formal_logical_failure"].astype(bool))[baseline_failure]
        ),
...
def run_experiment(config: BenchmarkConfig) -> dict[str, Any]:
    ...
    for offset in range(config.seed_count):
        ...
        benchmark = FiveQubitLogicalBenchmark(config, seed)
    ...
    return {
        ...
        "code": {
            "name": "5-qubit perfect stabilizer code",
...
def main() -> None:
    ...
    parser.add_argument("--seed-start", type=int, default=510001)
    ...
    evidence_path = output_dir / "qdsv_qldpc_five_qubit_stabilizer_logical_evidence.json"
    summary_path = output_dir / "qdsv_qldpc_five_qubit_stabilizer_logical_summary.csv"
    metrics_path = output_dir / "qdsv_qldpc_five_qubit_stabilizer_logical_metrics.csv"
